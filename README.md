...
## Ручные правки относительно генератора

- `lib/ObjectSerializer.php` — при разборе ответа исключения `\InvalidArgumentException`
  из сеттеров игнорируются: ограничения из контракта запроса (диапазоны, nullable)
  не применяются к ответу сервера. Без этого одно значение вне контракта
  (например `boxCount: 0`) рушит разбор всего ответа. Файл добавлен
  в `.openapi-generator-ignore`, регенерация его не затрагивает. См. APIS-6461.
